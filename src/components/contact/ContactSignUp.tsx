...
const ContactSignup = () => {
  ...
  const backendURL = import.meta.env.VITE_BACKEND_URL; // Use Amplify environment variable

  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch(`${backendURL}/api/package-list`);
        const data = await response.json();
        setPackages(data);
      } catch (err) {
        console.error("Error fetching packages:", err);
    ...
    <div className="min-h-screen bg-white font-sans relative flex flex-col">
      ...
      <section className="w-full bg-yellow-50 py-10 px-6 md:px-20 flex flex-col md:flex-row items-center justify-center">
        ...
        <div className="md:w-1/2 md:pl-12 text-center md:text-left">
          <h1 className="text-4xl font-bold text-[#6B4E3D] mb-4">
            Contact Sanchar6t - Plan Your Sacred Tirupati Journey
          </h1>
          <p className="text-lg text-gray-700 mb-4 leading-relaxed">
            Ready to embark on your spiritual journey? <strong>Sanchar6T</strong> makes planning your <strong>Tirupati trip from Bangalore</strong> simple, convenient, and completely stress-free. Our experienced customer service team is available to assist with bookings, answer questions, and customize packages according to your specific requirements.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-6 md:px-20 bg-yellow-50">
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center">
          {/* Left: Logo/Image */}
          <div className="md:w-1/2 flex flex-col items-center md:items-start">
            <img src={Logo} alt="tirupati trip from bangalore" className="h-72 w-auto rounded-xl" />
            <p className="mt-6 text-center md:text-left text-gray-700 text-lg leading-relaxed max-w-md">
              Connect with Tirupati travel experts and plan your sacred journey from Bangalore with ease. 
              Our experienced team will guide you through bookings, package selection, and provide all the assistance you need for a smooth and memorable pilgrimage.
            </p>
