...
const TOUR_OPTIONS = [
  {
    id: "tirupati1n1d",
    label: "APTDC Tirupati 1 Night / 1 Day Dharma Darshan Package",
    url: "/tirupati-package-from-bangalore"
  },
  {
    id: "tirupatiSrikalahasti",
    label: "Divine Blessings & Sacred Serenity – Tirupati & Srikalahasti in 2 Days 2 Nights",
    url: "/tirupati-srikalahasti"
  }
];
...
export default function ItineraryPlanner() {
  ...
  const handleRedirect = () => {
    const selectedTour = TOUR_OPTIONS.find(t => t.id === tourId);
    if (!selectedTour) return;
    toast(`Redirecting to "${selectedTour.label}"...`);
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      window.location.href = selectedTour.url;
    }, 1000); // show spinner for 1 sec before redirect
  };
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 font-sans relative">
      ...
      <div className="bg-gradient-to-r from-[#05386B] via-[#136F63] to-[#B2DBBF] rounded-3xl p-6 shadow-2xl text-white">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-1 gap-4">
          <div>
            <label className="text-xl font-semibold">Select Package</label>
            <select
              value={tourId}
              onChange={(e) => setTourId(e.target.value)}
              className="mt-1 w-full p-3 rounded-xl text-gray-800 shadow focus:ring-2 focus:ring-green-400"
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          >
            {loading ? (
              <>
                <RefreshCw className="animate-spin" size={16} /> Redirecting...
              </>
            ) : (
              <>
                <Zap size={16} /> Go to Package
              </>
            )}
          </button>
        </div>
