...
//   // ✅ Enhanced Input Change Handler with validation + female-seat toasts
//   const handleInputChange = (index: number, field: string, value: any) => {
//     const seat = selectedSeats[index];
//     const seatNo = seat?.SeatNo || seat?.seatNumber || seat?.id || seat;
//     const isWomenSeat = femaleSeats.map(String).includes(String(seatNo));

//     // 🩷 If seat is female-only and user clicks it (first time gender select)
//     // if (isWomenSeat && !formData[index]?.gender) {
//     //   toast.success(`Seat ${seatNo} is reserved for women passengers only.`);
//     // }

//     // 🚫 Validation: block male selection on pink seats
//     if (field === "gender") {
//       if (isWomenSeat && value === "male") {
//         toast.error(
//           `Seat ${seatNo} is reserved for women only. Please select Female or choose another seat.`
...
//       // ✅ Auto-toast if user selects female for pink seat
//       if (isWomenSeat && value === "female") {
//         toast.success(`Seat ${seatNo} confirmed for a female passenger.`);
//       }
//     }
...
//       {selectedSeats.map((seatId: string, index: number) => {
//         const seatNo = seatId?.toString();
//         const isWomenSeat = femaleSeats.map(String).includes(seatNo);
//         return (
//           <div key={seatId} className="mb-8">
//             <div className="flex items-center mb-4 justify-between">
//               <div className="flex items-center">
...
import maleImg from "@/assets/male.png";
import femaleImg from "@/assets/female.png";
import maleImg1 from "@/assets/male1 (2).png";
import femaleImg2 from "@/assets/female1 (2).png";
import { AlertTriangle } from "lucide-react";
import { toast } from "react-hot-toast";
